...
int WINAPI _tWinMain(HINSTANCE hinstExe, HINSTANCE, PTSTR pszCmdLine, int)
{
   ...
   else
   {
      ...
      a = DialogBoxParam(hinstExe, MAKEINTRESOURCE(IDD_ERRORSHOW),
                         NULL, Dlg_Proc, _ttoi(pszCmdLine));
      dwError = GetLastError();
   }
   HLOCAL hlocal1 = NULL; // Buffer that gets the error message string

   // Use the default system locale since we look for Windows messages.
   // Note: this MAKELANGID combination has 0 as value
   DWORD systemLocale2 = MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL);

   // Get the error code's textual description
   BOOL fOk1 = FormatMessage(
       FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS |
           FORMAT_MESSAGE_ALLOCATE_BUFFER,
       NULL, dwError, systemLocale2,
       (PTSTR)&hlocal1, 0, NULL);
   MessageBox(NULL, (LPWSTR)hlocal1, L"窗口消息", 0);
   // system("pause");
   return (a);
}
